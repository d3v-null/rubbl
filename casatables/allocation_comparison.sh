...
echo ""
echo "3. DIFFERENCES BETWEEN RUST FFI AND C++ DIRECT:"
echo "   - Rust FFI: Higher syscall count (709 vs 389 zero-writes)"
echo "   - C++ Direct: Lower syscall count but same inefficient pattern"
echo "   - Both exhibit the same fundamental inefficiency: explicit zero-writing"
echo ""
echo "4. EFFICIENT ALTERNATIVE WOULD SHOW:"
...
echo
echo "=== Next Steps ==="
echo "1. Modify casacore's BucketCache::extend() to use ftruncate pre-allocation"
echo "2. Update TSMCube::initCallBack() to avoid unnecessary zero-filling"
