...
echo "   - C++:  389 zero-write operations of 3328 bytes each, 1198 lseek operations"
echo "   - Pattern: lseek(offset) -> write(3328 bytes of zeros) -> repeat"
echo "   - With CASACORE_DISABLE_PREALLOC=1: Rust seeks 1518 ftruncate=0; C++ ftruncate=33"
echo ""
...
echo "   - Rust FFI: Higher zero-write count (709 vs 389). ftruncate appears only when prealloc is enabled"
echo "   - C++ Direct: Lower zero-write count and ftruncate visible regardless of CASACORE_DISABLE_PREALLOC"
echo "   - Both exhibit the same fundamental inefficiency: explicit zero-writing; prealloc reduces seeks"
echo ""
